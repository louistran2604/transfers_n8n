...
`womens-football-blacklist.txt` is the authoritative one-name-or-variant-per-line exclusion list. `entity-aliases.json` contains `clubs`, global `players`, current-club-scoped `enrichment_player_aliases`, exact `sibling_groups`, and normalized `common_surnames`. Qwen is instructed to preserve stated given names for common surnames, but the JavaScript digest filter is authoritative. Current requests precede historical retries under the 25-item enrichment cap; retries still only piggyback on runs that reach the enrichment query. Regenerate and re-import after registry edits; never edit generated workflow JSON directly.
