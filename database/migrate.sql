\set ON_ERROR_STOP on

SELECT pg_advisory_lock(hashtext('transfers_net_schema_migrations'));

CREATE TABLE IF NOT EXISTS app_schema_migrations (
  version text PRIMARY KEY,
  applied_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP
);

SELECT CASE
  WHEN EXISTS (
    SELECT 1
    FROM app_schema_migrations
    WHERE version = '001_initial_schema'
  ) THEN 'false'
  ELSE 'true'
END AS migration_001_pending \gset

\if :migration_001_pending
  \i /database/migrations/001_initial_schema.sql
  INSERT INTO app_schema_migrations (version) VALUES ('001_initial_schema');
\endif

SELECT CASE
  WHEN EXISTS (
    SELECT 1
    FROM app_schema_migrations
    WHERE version = '002_soccerdata_enrichment'
  ) THEN 'false'
  ELSE 'true'
END AS migration_002_pending \gset

\if :migration_002_pending
  BEGIN;
  \i /database/migrations/002_soccerdata_enrichment.sql
  INSERT INTO app_schema_migrations (version) VALUES ('002_soccerdata_enrichment');
  COMMIT;
\endif

SELECT CASE
  WHEN EXISTS (
    SELECT 1
    FROM app_schema_migrations
    WHERE version = '003_transfer_probability'
  ) THEN 'false'
  ELSE 'true'
END AS migration_003_pending \gset

\if :migration_003_pending
  BEGIN;
  \i /database/migrations/003_transfer_probability.sql
  INSERT INTO app_schema_migrations (version) VALUES ('003_transfer_probability');
  COMMIT;
\endif

SELECT CASE
  WHEN EXISTS (
    SELECT 1
    FROM app_schema_migrations
    WHERE version = '004_probability_v1_engine'
  ) THEN 'false'
  ELSE 'true'
END AS migration_004_pending \gset

\if :migration_004_pending
  BEGIN;
  \i /database/migrations/004_probability_v1_engine.sql
  INSERT INTO app_schema_migrations (version) VALUES ('004_probability_v1_engine');
  COMMIT;
\endif

SELECT CASE
  WHEN EXISTS (
    SELECT 1
    FROM app_schema_migrations
    WHERE version = '005_probability_v1_normalization'
  ) THEN 'false'
  ELSE 'true'
END AS migration_005_pending \gset

\if :migration_005_pending
  BEGIN;
  \i /database/migrations/005_probability_v1_normalization.sql
  INSERT INTO app_schema_migrations (version) VALUES ('005_probability_v1_normalization');
  COMMIT;
\endif

SELECT CASE
  WHEN EXISTS (
    SELECT 1
    FROM app_schema_migrations
    WHERE version = '006_probability_backfill'
  ) THEN 'false'
  ELSE 'true'
END AS migration_006_pending \gset

\if :migration_006_pending
  BEGIN;
  \i /database/migrations/006_probability_backfill.sql
  INSERT INTO app_schema_migrations (version) VALUES ('006_probability_backfill');
  COMMIT;
\endif

SELECT CASE
  WHEN EXISTS (
    SELECT 1 FROM app_schema_migrations
    WHERE version = '007_probability_active_digest'
  ) THEN 'false' ELSE 'true'
END AS migration_007_pending \gset

\if :migration_007_pending
  BEGIN;
  \i /database/migrations/007_probability_active_digest.sql
  INSERT INTO app_schema_migrations (version) VALUES ('007_probability_active_digest');
  COMMIT;
\endif

SELECT CASE
  WHEN EXISTS (
    SELECT 1 FROM app_schema_migrations
    WHERE version = '008_fee_context'
  ) THEN 'false' ELSE 'true'
END AS migration_008_pending \gset

\if :migration_008_pending
  BEGIN;
  \i /database/migrations/008_fee_context.sql
  INSERT INTO app_schema_migrations (version) VALUES ('008_fee_context');
  COMMIT;
\endif

SELECT CASE
  WHEN EXISTS (
    SELECT 1 FROM app_schema_migrations
    WHERE version = '009_probability_outcome_settlement'
  ) THEN 'false' ELSE 'true'
END AS migration_009_pending \gset

\if :migration_009_pending
  BEGIN;
  \i /database/migrations/009_probability_outcome_settlement.sql
  INSERT INTO app_schema_migrations (version) VALUES ('009_probability_outcome_settlement');
  COMMIT;
\endif

SELECT CASE
  WHEN EXISTS (
    SELECT 1 FROM app_schema_migrations
    WHERE version = '010_probability_final_review_fixes'
  ) THEN 'false' ELSE 'true'
END AS migration_010_pending \gset

\if :migration_010_pending
  BEGIN;
  \i /database/migrations/010_probability_final_review_fixes.sql
  INSERT INTO app_schema_migrations (version) VALUES ('010_probability_final_review_fixes');
  COMMIT;
\endif

SELECT CASE
  WHEN EXISTS (
    SELECT 1 FROM app_schema_migrations
    WHERE version = '011_probability_final_remediation'
  ) THEN 'false' ELSE 'true'
END AS migration_011_pending \gset

\if :migration_011_pending
  BEGIN;
  \i /database/migrations/011_probability_final_remediation.sql
  INSERT INTO app_schema_migrations (version) VALUES ('011_probability_final_remediation');
  COMMIT;
\endif

SELECT pg_advisory_unlock(hashtext('transfers_net_schema_migrations'));
