...
const identity = (id, name) => ({
  ...
  canonical_name: name, score: 80, margin: 80, resolver_version: 'identity-v3',
});
...
const item = ({ key, reportId, playerId, name, teamId, teamName, competitionId, seasonId }) => ({
  item_key: key, report_ids: [String(reportId)], status: 'fresh', resolver_version: 'identity-v3',
...
const aliasPayload = payload('generated-alias', [
  {
    item_key: 'bruno-a', report_ids: ['900575'], status: 'partial', resolver_version: 'identity-v3',
  ...
  {
    item_key: 'bruno-b', report_ids: ['900585'], status: 'partial', resolver_version: 'identity-v3',
    retryable: false, provider_calls: 1, cache_hits: 0,
    request_context: { reported_name_key: 'bruno guimarães' }, identity: identity('866469', 'Bruno Guimarães'),
    profile: profile('866469', 'Bruno Guimarães', '9702', 'Alias Team'), statistics: null, candidates: [], warning_codes: [], error: null,
  },
...
) SELECT id, provider_id, display_name, 'automatic', 'identity-v3', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
...
) SELECT 900610, provider.id, 'automatic', 'identity-v3', CURRENT_TIMESTAMP
...
INSERT INTO player_enrichment_attempts (
  ...
  ('fixture-old-version', 'fixture-old-version', 'old-version', 900077, 900605,
   'unresolved', true, CURRENT_TIMESTAMP + interval '24 hours', '{}', '{"resolver_version":"identity-v3"}', CURRENT_TIMESTAMP - interval '1 hour', CURRENT_TIMESTAMP - interval '1 hour'),
  ...
  ('fixture-due-transient', 'fixture-due-transient', 'due-transient', 900077, 900614,
   'provider_failure', true, CURRENT_TIMESTAMP - interval '1 minute', '{}', '{"resolver_version":"identity-v3"}', CURRENT_TIMESTAMP - interval '5 minutes', CURRENT_TIMESTAMP - interval '5 minutes'),
  ('fixture-future-transient', 'fixture-future-transient', 'future-transient', 900077, 900615,
   'timeout', true, CURRENT_TIMESTAMP + interval '5 minutes', '{}', '{"resolver_version":"identity-v3"}', CURRENT_TIMESTAMP - interval '5 minutes', CURRENT_TIMESTAMP - interval '5 minutes');

PREPARE generated_context(jsonb, text) AS
${contextNode.parameters.query};
EXECUTE generated_context('[]'::jsonb, '900077') \\gset context_
...
UPDATE player_enrichment_attempts
SET evidence = jsonb_set(evidence, '{resolver_version}', '"identity-v2"')
...
BEGIN
  ...
  IF (SELECT count(*) FROM player_enrichment_attempts WHERE batch_request_key = 'generated-alias' AND evidence->>'resolver_version' = 'identity-v3') <> 2 THEN RAISE EXCEPTION 'attempt resolver version missing'; END IF;
END
$check$;
